...
def tass_points(arr: list, index_to_tass_arr: list):
    """
    Randomly tass points
    """
    move = 0.008
    for i in index_to_tass_arr:
        x_coef = uniform(-1,1)
        y_coef = uniform(-1,1)
        current_location = arr[i][0]
        new_location = [0,0]
        new_location[0] = current_location[0] + x_coef * move
        new_location[1] = current_location[1] + y_coef * move
        arr[i][0] = tuple(new_location)
...
with open(path_to_dataset,"r",encoding = "utf-8") as film_spots:
    ...
    html_map = folium.Map(location = [latitude, longitude], zoom_start=3)
    ...
    fg_near_film_area.add_child(folium.Circle(location = [latitude, longitude],
                                    ...
                                    fill_color = '#040',
                                    radius = (nearest[-1][-1] + 100)*1000,
                                    weight = 1,
                                    color = "#004"))
    html_map.add_child(fg_near_film_spot)
    html_map.add_child(fg_near_film_area)
    html_map.add_child(folium.LayerControl())
    html_map.save('Map_Default.html')
